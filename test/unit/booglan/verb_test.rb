...
class VerbTest < Test::Unit::TestCase
  ...
  def test_nil_paramenters
    result = Booglan::Verb.new.perform(nil)
    assert_equal "This text has 0 verb and 0 inflected verb", result
  end

  def test_no_verb
    result = Booglan::Verb.new.perform('acc')
    assert_equal "This text has 0 verb and 0 inflected verb", result
  end

  def test_has_more_than_one_verb_and_inflected_verbs
    result = Booglan::Verb.new.perform(SAMPLE_TEXT)
    assert_equal "This text has 71 verbs and 58 inflected verbs", result
  end

  def test_has_more_than_one_verb_and_none_inflected_verbs
    result = Booglan::Verb.new.perform('blnsnhgf rtpffmcf')
    assert_equal "This text has 2 verbs and 0 inflected verb", result
  end
end